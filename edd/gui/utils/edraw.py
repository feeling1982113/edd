...
class EDraw():
    ...
    DefaultNodeFont = QFont('Helvetica', 8, False)
    
    class EColor():

        DefaultSceneFillColor = QColor(92, 102, 114)
        #DefaultSceneFillColor = QColor(46, 47, 51)
        DefaultSceneGridColor = QColor(80, 89, 100)
        ...
        DefaultEnterHoverPen = QPen(QColor(247, 147, 30), 1, Qt.SolidLine) # F7931E
    ...
    @staticmethod
    def ResourceHandle(radius):
        p1 = EDraw.Pie(180-45, 180+45, 4, radius + 5, 0, 24, 0 )
        p2 = EDraw.Circle(radius, 50).united( EDraw.Pie( 360-45, 45, 4, radius + 5, 0, 24, 0 ) )
        return p1.united(p2)

    @staticmethod
    def Collection(title, QFont):

        fm = QFontMetrics(QFont).boundingRect(title)

        print fm

        radius = 20

        rect = QRectF( -fm.width() / 2, -radius, fm.width(), radius * 2)

        tPoly = QPolygonF(rect)

        rsOne = EDraw.ResourceHandle(radius)
        rsTwo = EDraw.ResourceHandle(radius)

        rsOne = rsOne.translated(rect.width() / 2, 0)
        rsTwo = rsTwo.translated(-rect.width() / 2, 0)

        return rsOne.united(tPoly).united(rsTwo)
